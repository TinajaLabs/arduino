...
void loop(){
  ...
  if (mySerialVal != "n" ) {
    ...
    // if the first character is a B, a beat from the other system
    if (mySerialVal[0] == 'B')  setRemoteLED(mySerialVal);
...
void setRemoteLED(char* serialCode){
  // Serial.println(strlen(mySerialVal));

  String beatString = String(serialCode[1]) + String(serialCode[2]) + String(serialCode[3]);
  // Serial.print("from other: ");
  // Serial.println(beatString);

  int beatValue = beatString.toInt();
